...
CREATE TABLE IF NOT EXISTS system_configs (
    ...
    field_type VARCHAR(20) NOT NULL DEFAULT 'text' COMMENT 'UI field type: text, number, password, textarea, select, radio, boolean, time',
    field_options TEXT NULL COMMENT 'JSON options for select/radio: [{"value":"","label":""}]',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (id),
...
INSERT IGNORE INTO system_configs (id, config_key, config_value, config_group, description, field_type, field_options) VALUES
('00000000-0000-0000-0000-000000000001', 'smtp.host', 'smtp.gmail.com', 'SMTP', 'SMTP server host', 'text', NULL),
('00000000-0000-0000-0000-000000000002', 'smtp.port', '587', 'SMTP', 'SMTP server port', 'number', NULL),
('00000000-0000-0000-0000-000000000003', 'smtp.username', '', 'SMTP', 'SMTP username for authentication', 'text', NULL),
('00000000-0000-0000-0000-000000000004', 'smtp.password', '', 'SMTP', 'SMTP password for authentication', 'password', NULL),
('00000000-0000-0000-0000-000000000005', 'smtp.from-email', '', 'SMTP', 'Sender email address', 'text', NULL),
('00000000-0000-0000-0000-000000000006', 'otp.expiry_minutes', '5', 'OTP', 'OTP expiry time in minutes', 'number', NULL),
('00000000-0000-0000-0000-000000000007', 'otp.length', '6', 'OTP', 'OTP code length', 'number', NULL),
('00000000-0000-0000-0000-000000000008', 'auth.allowed_domains', 'dntg.com.vn', 'AUTH', 'Comma-separated allowed email domains', 'text', NULL),
('00000000-0000-0000-0000-000000000009', 'auth.token_expiry_hours', '24', 'AUTH', 'Admin session token expiry in hours', 'number', NULL),
('00000000-0000-0000-0000-000000000010', 'auth.login_mode', 'auto', 'AUTH', 'Admin login mode: otp = OTP only, password = password only, auto = both with SMTP fallback', 'select', '[{"value":"otp","label":"OTP only"},{"value":"password","label":"Password only"},{"value":"auto","label":"Auto (fallback)"}]'),
('00000000-0000-0000-0000-000000000011', 'auth.password_setup', 'open', 'AUTH', 'Allow first-time password creation without login: open = allowed once per account, closed = disabled', 'select', '[{"value":"open","label":"Open"},{"value":"closed","label":"Closed"}]'),
('00000000-0000-0000-0000-000000000012', 'auth.super_admin_email', 'long.hg@dntg.com.vn', 'AUTH', 'Email auto-assigned SUPER_ADMIN role on login', 'text', NULL),
('00000000-0000-0000-0000-000000000013', 'auth.super_admin_role', 'SUPER_ADMIN', 'AUTH', 'Role name assigned to the super admin email', 'text', NULL),
('00000000-0000-0000-0000-000000000014', 'auth.default_role', 'VIEWER', 'AUTH', 'Default role assigned on first login', 'text', NULL),
('00000000-0000-0000-0000-000000000015', 'auth.max_failed_attempts', '5', 'AUTH', 'Password login failures before temporary lock', 'number', NULL),
('00000000-0000-0000-0000-000000000016', 'auth.fail_window_minutes', '10', 'AUTH', 'Sliding window in minutes for counting login failures', 'number', NULL),
('00000000-0000-0000-0000-000000000017', 'auth.lock_minutes', '10', 'AUTH', 'Account lock duration in minutes after too many failures', 'number', NULL),
('00000000-0000-0000-0000-000000000018', 'auth.password_min_length', '8', 'AUTH', 'Minimum password length (API validation allows 1-72, effective is max of both)', 'number', NULL),
('00000000-0000-0000-0000-000000000019', 'auth.password_max_length', '72', 'AUTH', 'Maximum password length (BCrypt limit is 72)', 'number', NULL),
('00000000-0000-0000-0000-000000000020', 'otp.resend_cooldown_seconds', '30', 'OTP', 'Minimum seconds between OTP requests per email', 'number', NULL),
('00000000-0000-0000-0000-000000000021', 'smtp.conn_timeout_ms', '5000', 'SMTP', 'SMTP connection timeout in milliseconds', 'number', NULL),
('00000000-0000-0000-0000-000000000022', 'smtp.read_timeout_ms', '5000', 'SMTP', 'SMTP read timeout in milliseconds', 'number', NULL),
('00000000-0000-0000-0000-000000000023', 'smtp.starttls_enable', 'true', 'SMTP', 'Enable STARTTLS for SMTP', 'boolean', NULL),
('00000000-0000-0000-0000-000000000024', 'smtp.otp_subject', '[WalletCenTral] Admin Login OTP Code', 'SMTP', 'Subject of the OTP email', 'text', NULL),
('00000000-0000-0000-0000-000000000025', 'alert.large_transaction_threshold', '1000000', 'ALERT', 'Amount in VND triggering a large-transaction alert', 'number', NULL),
('00000000-0000-0000-0000-000000000026', 'alert.low_balance_threshold', '100000', 'ALERT', 'Wallet balance in VND below which a low-balance alert fires', 'number', NULL),
('00000000-0000-0000-0000-000000000027', 'alert.large_transaction_title', 'Cảnh báo giao dịch lớn', 'ALERT', 'Title of the large-transaction notification', 'text', NULL),
('00000000-0000-0000-0000-000000000028', 'alert.large_transaction_message', 'Giao dịch {0} với số tiền {1} VND trên ví {2}', 'ALERT', 'Message template ({0}=id, {1}=amount, {2}=wallet)', 'textarea', NULL),
('00000000-0000-0000-0000-000000000029', 'alert.low_balance_title', 'Cảnh báo số dư thấp', 'ALERT', 'Title of the low-balance notification', 'text', NULL),
('00000000-0000-0000-0000-000000000030', 'alert.low_balance_message', 'Ví {0} số dư còn lại: {1} VND', 'ALERT', 'Message template ({0}=wallet, {1}=balance)', 'textarea', NULL),
('00000000-0000-0000-0000-000000000031', 'billing.default_description_template', 'Thanh toán cước {0}', 'BILLING', 'Default billing description ({0}=service code)', 'textarea', NULL),
...
CREATE TABLE IF NOT EXISTS admin_users (
    ...
    password_hash VARCHAR(255) NULL COMMENT 'BCrypt password hash for password login (NULL = not set)',
